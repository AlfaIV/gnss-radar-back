...
func (mr *MeasurementsRepo) UploadEphemeris(ctx context.Context, req measurements_domain.EphemerisToLoad) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to load Moscow time location")
	}
	createdAt := time.Now().In(loc)

	_, err = mr.minio.PutObject(
		ctx,
		bucketName,
		req.MinioName,
		req.Payload,
	...
	writeFileMeta := `
        INSERT INTO file_meta(filename, minio_name, created_at) 
        VALUES ($1, $2, $3)
    `

	_, err = mr.pool.Exec(ctx, writeFileMeta, req.Name, req.MinioName, createdAt)
	if err != nil {
		delErr := mr.minio.RemoveObject(
			ctx,
			bucketName,
			req.MinioName,
		...
		if delErr != nil {
			return errors.Wrap(delErr, "ALARM! MinIO cleanup failed after DB error")
		}
		return errors.Wrap(err, "rolled back MinIO object due to DB failure")
	}
...
func (mr *MeasurementsRepo) GetSatellitesCoordinates(ctx context.Context) (measurements_domain.Satellites, error) {
	loc, err := time.LoadLocation("Europe/Moscow")
	if err != nil {
		return measurements_domain.Satellites{}, errors.Wrap(err, "failed to load Moscow time location")
	}
